...
cliplen_sec=15
max_tries=5
# indir="/mnt/data/share/ego4d_data/v2/full_scale/"
indir="/mnt/data/share/sss/annofab-data/20230401"
outdir="/mnt/data/zhu/AVION/datasets/clip_dummy/"
...
for video in "${all_videos[@]}"; do
	...
	while [ $try -le $max_tries ]; do
            ffmpeg -y -ss ${st_sec} -i "${indir}/${video}" ${scale_str} -t ${cliplen_sec} "${outfpath}" -hide_banner -loglevel error -movflags faststart
	    try=$(( $try + 1 ))
