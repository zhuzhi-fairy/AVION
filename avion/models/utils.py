...
def enable_grad_checkpointing(model: nn.Module, enable: bool):
    if hasattr(model, "set_grad_checkpointing"):
        ...
        print(
            "{} has no attribute named 'set_grad_checkpointing'".format(
                model._get_name()
            )
        )
...
def remap_keys_from_open_clip_to_vit(
    ...
    for layer in range(visual_transformer_layers):
        if use_flash_attn:
            for src_name, tgt_name in {
                "attn.in_proj_weight": "attn.Wqkv.weight",
                "attn.in_proj_bias": "attn.Wqkv.bias",
                ...
                key_mapping[
                    f"visual.transformer.resblocks.{layer}.{src_name}"
                ] = f"visual.transformer.resblocks.{layer}.{tgt_name}"
    ...
    for layer in range(textual_transformer_layers):
        for name in [
            ...
            key_mapping[f"transformer.resblocks.{layer}.{name}"] = (
                f"textual.transformer.resblocks.{layer}.{name}"
            )
    ...
    for key in clip_state_dict:
        ...
        if use_fast_conv1 and key == "visual.conv1.weight":
            remapped_state_dict["visual.conv1.weight"] = clip_state_dict[
                key
            ].flatten(1)
            ...
            if key == "positional_embedding":
                ...
                if context_length <= old_context_length:
                    remapped_state_dict[key_mapping[key]] = clip_state_dict[
                        key
                    ][:context_length, :]
                else:
                    remapped_state_dict[key_mapping[key]] = torch.cat(
                        (
                            clip_state_dict[key],
                            torch.zeros(
                                (context_length - old_context_length, dim),
                                dtype=old_dtype,
                            ),
                        ),
                        dim=0,
                ...
                remapped_state_dict[key_mapping[key]] = torch.cat(
                    (
                        clip_state_dict[key],
                        torch.zeros(
                            (vocab_size - old_vocab_size, dim), dtype=old_dtype
                        ),
                    ),
                    dim=0,
                )
...
def inflate_positional_embeds(
    ...
    temporal_embedding_key="visual.temporal_embedding",
):
    # allow loading of timesformer with fewer num_frames
    curr_keys = list(current_model_state_dict.keys())
    if (
        temporal_embedding_key in new_state_dict
        and temporal_embedding_key in curr_keys
    ):
        load_temporal_embed = new_state_dict[temporal_embedding_key]
        ...
        if load_num_frames != curr_num_frames:
            ...
            new_state_dict[temporal_embedding_key] = new_temporal_embed
    # allow loading with smaller spatial patches. assumes custom border crop, to append the
    # border patches to the input sequence
    if (
        "visual.positional_embedding" in new_state_dict
        and "visual.positional_embedding" in curr_keys
    ):
        load_pos_embed = new_state_dict["visual.positional_embedding"]
        load_num_patches = load_pos_embed.shape[0]
        curr_pos_embed = current_model_state_dict[
