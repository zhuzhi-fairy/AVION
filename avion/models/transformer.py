...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        if not use_flash_attn:
            self.attn = nn.MultiheadAttention(
                d_model, n_head, dropout=attn_drop
            )
        else:
            self.attn = FlashMHA(
                d_model,
        ...
        if not use_flash_attn:
            ...
            self.mlp = FlashMlp(
                d_model, hidden_features=mlp_width, activation=act_layer()
            )
        self.ls_2 = (
            LayerScale(d_model, ls_init_value)
            if ls_init_value is not None
    ...
    def forward(
        ...
        if not self.use_flash_attn:
            x = x + self.drop_path(
                self.ls_1(self.attention(self.ln_1(x), attn_mask=attn_mask))
            )
        else:
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        for r in self.resblocks:
            ...
            # nl += 1
            # print(
            #     "large {}, output max {:.04f}".format(
            #         nl, x.max().detach().cpu().numpy()
            #     )
            # )
        return x
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(
            scale
            * torch.randn(self.grid_size[0] * self.grid_size[1] + 1, width)
        )
        assert num_frames >= 1
        ...
        assert not (patch_dropout > 0.0 and drop_rate > 0.0)
        # setting a patch_dropout of 0. would mean it is disabled and this function would be the identity fn
        self.patch_dropout = (
            PatchDropout(patch_dropout)
            if patch_dropout > 0.0
            else nn.Identity()
        )
        self.pos_drop = (
            nn.Dropout(p=drop_rate) if drop_rate > 0.0 else nn.Identity()
        )
            ...
            for n, p in block.named_parameters():
                if "weight" in n:
                    ...
                    raise NotImplementedError(
                        "Unknown parameters named {}".format(n)
                    )
...
class TextTransformer(nn.Module):
    ...
    def forward(self, text, cast_dtype=None):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(
            x, attn_mask=self.attn_mask if self.causal_mask else None
        )
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x)
        ...
        ] @ self.text_projection.to(cast_dtype)
