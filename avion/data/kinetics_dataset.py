...
def video_loader_by_frames(
    ...
    try:
        ...
        frames = np.zeros((len(frame_ids), height, width, 3))
...
class KineticsDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if len(self.samples) != num_samples:
            self.samples = random.choices(self.samples, k=num_samples)
    ...
    def __getitem__(self, i):
        if self.verbose:
            print(
                "[{}] __getitem__() starts at {}".format(
                    os.getpid(), datetime.datetime.now()
                )
            )
        ...
        if num_frames > (self.clip_length + 1) * self.clip_stride:
            start_id = np.random.randint(
                num_frames - (self.clip_length + 1) * self.clip_stride
            )
        else:
            start_id = 0
        frame_ids = np.arange(
            start_id,
            start_id + self.clip_length * self.clip_stride,
            step=self.clip_stride,
        )
        if self.is_training:
        ...
        if self.is_training:
            frames = video_loader_by_frames(
                self.root,
                video_id + ".mp4" if "." not in video_id else video_id,
                frame_ids,
                threads=self.threads,
                fast_rrc=self.fast_rrc,
                rrc_params=self.rrc_params,
                fast_msc=self.fast_msc,
                msc_params=self.msc_params,
                fast_cc=False,
        ...
        if self.verbose:
            print(
                "[{}] __getitem__() end at {}".format(
                    os.getpid(), datetime.datetime.now()
                )
            )

        if self.masked_position_generator is None:
            return frames, video_id, frame_ids
        else:
