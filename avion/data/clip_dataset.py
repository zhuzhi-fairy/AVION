...
def get_frame_ids(start_frame, end_frame, num_segments=32, jitter=True):
    frame_ids = np.convolve(
        np.linspace(start_frame, end_frame, num_segments + 1),
        [0.5, 0.5],
        mode="valid",
    )
    if jitter:
...
def video_loader(
    ...
    if chunk_len == -1:
        ...
        for chunk in range(chunk_start, chunk_end + chunk_len, chunk_len):
            rel_frame_ids = list(
                filter(
                    lambda x: int(chunk * fps)
                    <= x
                    < int((chunk + chunk_len) * fps),
                    frame_ids,
                )
            )
            rel_frame_ids = [
                int(frame_id - chunk * fps) for frame_id in rel_frame_ids
            ]
            vr = get_video_reader(
                osp.join(
                    root, "{}.{}".format(vid, ext), "{}.{}".format(chunk, ext)
                ),
                num_threads=threads,
                fast_rrc=fast_rrc,
                rrc_params=rrc_params,
        ...
        res = torch.from_numpy(
            np.concatenate(all_frames, axis=0).astype(np.float32)
        )
        assert (
            res.shape[0] == clip_length
        ), "{}, {}, {}, {}, {}, {}, {}".format(
            root,
            vid,
            second,
...
class VideoCaptionDatasetBase(torch.utils.data.Dataset):
        ...
        if self.dataset == "ego4d":
            ...
            with open(metadata) as f:
                ...
                for row in csv_reader:
                    pid, vid = row[1:3]
                    start_timestamp, end_timestamp = datetime2sec(
                        row[4]
                    ), datetime2sec(row[5])
                    narration = row[8]
            ...
            if self.dataset == "ek100_mir":
                ...
                if "train" in metadata:
                    ...
                    raise ValueError(
                        '{} should contain either "train" or "test"!'.format(
                            metadata
                        )
                    )
                self.relevancy = 0.1
    ...
    def get_raw_item(
        ...
        if self.dataset == "ego4d":
            ...
            if is_training:
                positive_list = np.where(
                    self.relevancy_mat[i] > self.relevancy
                )[0].tolist()
                if positive_list != []:
                    pos = random.sample(
                        positive_list, min(len(positive_list), 1)
                    )[0]
                    if (
