...
def get_args_parser():
    parser = argparse.ArgumentParser(
        description="AVION pretrain", add_help=False
    )
    parser.add_argument(
        "--dataset", default="ego4d", type=str, choices=["ego4d"]
    )
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        "--clip-length", default=4, type=int, help="clip length"
    )
    parser.add_argument(
        "--clip-stride", default=16, type=int, help="clip stride"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_false",
        dest="freeze_temperature",
    )
    parser.set_defaults(freeze_temperature=False)
    parser.add_argument(
        "--use-fast-conv1", action="store_true", dest="use_fast_conv1"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use-flash-attn", action="store_true", dest="use_flash_attn"
    )
    parser.add_argument(
...
def main(args):
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if checkpoint["args"].clip_length != args.clip_length:
                ...
                load_temporal_embedding = load_temporal_embedding.unsqueeze(
                    0
                ).permute(0, 2, 1)
                new_temporal_embed = (
                    F.interpolate(
                        load_temporal_embedding,
                        size=(args.clip_length,),
                        mode="linear",
                    )
                    .permute(0, 2, 1)
                    .squeeze(0)
    ...
    if args.evaluate:
        val_stats = validate_mir(
            val_loader, val_transform_gpu, model, criterion, args
        )
...
def train(
    ...
    for data_iter, inputs in enumerate(train_loader):
        ...
        inputs = [
            tensor.cuda(args.gpu, non_blocking=True) for tensor in inputs
        ]
        _ = (
            inputs.pop()
        ...
        if args.update_freq == 1:
            ...
            with torch.no_grad():
                with amp.autocast(enabled=not args.disable_amp):
                    chunk_image_features, chunk_text_features, _ = model(
                        images, texts
                    )
            ...
            for j in range(args.accum_freq):
                ...
                texts = accum_texts[j]
                with amp.autocast(enabled=not args.disable_amp):
                    chunk_image_features, chunk_text_features, logit_scale = (
                        model(images, texts)
                    )
...
def validate_mir(val_loader, transform_gpu, model, criterion, args):
    ...
    with amp.autocast(enabled=not args.disable_amp):
        with torch.no_grad():
            ...
            for i, inputs in enumerate(val_loader):
                ...
                inputs = [
                    tensor.cuda(args.gpu, non_blocking=True)
                ...
                for j in range(args.world_size):
                    all_video_embed[j].append(
                        gathered_image_features[j].detach().cpu()
                    )
                    all_text_embed[j].append(
                        gathered_text_features[j].detach().cpu()
                    )
                loss_dict = criterion(
                    image_features, text_features, logit_scale
                )
    ...
    text_id = pd.read_csv(
        args.val_metadata.replace("test", "test_sentence")
    ).values[:, 0]
    ...
    print(
        "mAP: V->T: {:.3f} T->V: {:.3f} AVG: {:.3f}".format(
            vis_map, txt_map, avg_map
        )
    )
